Below is a **full, self‑contained PyQt6 script** that:

- Shows a **single‑page PDF viewer** (page up/down, zoom, 1:1) with a left navigation dock.
- Has a **right dockable bookmark panel** implemented as a reusable class.
- If a PDF has **no bookmarks**, the bookmark dock lets users:
  - **Generate bookmarks automatically** (using a font‑size heuristic via PyMuPDF).
  - **Save** them:
    - back into the **same PDF**, or
    - into a **new PDF** with suffix `_with_bookmarks`.
- All bookmark‑related logic is encapsulated in:
  - `PDFBookmarkModel` (logic, no UI),
  - `PDFBookmarkDock` (UI; dockable; uses the model).
- These classes are **independent of the viewer** and can be reused in any other PDF viewer you build (including your continuous viewer).

Save as e.g. `PDFBookmarkViewer_Extended.py` and run:

```bash
python PDFBookmarkViewer_Extended.py
```
...
class PDFBookmarkModel(QWidget):
    ...
    - auto_generate_toc(font_threshold: float = 16.0) -> int
         -> generates a simple level-1 TOC from "large" text spans.
    ...
    def auto_generate_toc(self, font_threshold: float = 16.0) -> int:
        """
        Generate a simple TOC from text spans with font size >= font_threshold.

        All entries are level-1 bookmarks:
            [1, title, page_number]
                ...
                for line in block.get("lines", []):
                    for span in line.get("spans", []):
                        ...
                        if size >= font_threshold and any(c.isalpha() for c in text):
                            ...
                            # one heading per line is enough
                            break
    ...
    def save_inplace(self) -> None:
        ...
        # Non-incremental rewrite is simplest & robust
    ...
    def save_copy(self, suffix: str = "_with_bookmarks") -> str:
        ...
        self.modified = False
        # keep 'generated' flag so caller knows origin
        return out_path
...
- **No bookmarks → generate/save**  
  - Open a PDF that has no outline.  
  - Right dock shows `<No bookmarks> (use Generate)`.  
  - Click **Generate** in the bookmark dock toolbar; headings are detected and shown.  
  - Click **Save in PDF** to write them back into the same file, or **Save as _with_bookmarks** to create a copy.

- **Bookmark toolbar dockable**  
  - `PDFBookmarkDock` is a `QDockWidget`; you can move/float/hide it like any other dock.  
  - It is fully separated from the core viewer and can be reused with another viewer by wiring:
    - `bookmarkDock.set_model(yourModel)`
    - `bookmarkDock.bookmarkActivated.connect(yourViewer.go_to_page)`.

- **Scalable, reusable bookmark logic**  
  - All bookmark reading/writing/generation lives in `PDFBookmarkModel`.
  - You can import just `PDFBookmarkModel` and `PDFBookmarkDock` into your other scripts (e.g., continuous viewer) and attach them to any PyMuPDF `Document` and any viewer that has a `go_to_page(int)` method.