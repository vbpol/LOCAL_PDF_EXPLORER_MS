V4
Below is an updated, complete script that adds an **Adobe‑like view menu** (as in your screenshot) to the dockable `PDFViewerToolBar`:

Menu items included:

- Single‑page view (checkable)
- Two‑page view (checkable)
- Show cover page (present but disabled, like in Adobe)
- Enable scrolling (checkable)
- Actual size (checkable)
- Zoom to page level
- Fit to width
- Fit height
- Fit visible content
- Read mode (checkable)
- Full screen mode (checkable; Esc exits)
- OTHER TOOLS → Marquee zoom (checkable; simple marquee-zoom implementation)

The toolbar is still dockable on any side, and all menu options are wired to behavior in the main viewer.

Save as `PDFViewerToolBar3.py` and run:

```bash
python PDFViewerToolBar3.py
```
...
class PDFViewerToolBar(QDockWidget):
    ...
    - zoomOutRequested()
    - actualSizeRequested()
    - viewModeRequested(mode: str)        # "single" or "two"
    - scrollingToggled(enabled: bool)
    - zoomPageLevelRequested()
    - fitWidthRequested()
    - fitHeightRequested()
    - fitVisibleRequested()
    - readModeToggled(enabled: bool)
    - fullScreenToggled(enabled: bool)
    - marqueeZoomToggled(enabled: bool)
    """
    ...
    def _init_ui(self):
        ...
        self.act_show_cover.setEnabled(False)  # same as Acrobat (often disabled)
...
class PDFBMViewer(clsPDFBMViewer):
    ...
    - Left dock: PDFViewerToolBar (page + zoom + Adobe-like menu).
    ...
    def _create_top_toolbar(self):
        ...
        tb.setMovable(True)
        self.addToolBar(tb)
        self.main_toolbar = tb  # keep reference for read mode
    ...
    def _set_read_mode(self, enabled: bool):
        ...
        # Simple implementation: hide menu bar, toolbars, docks
    ...
    # ---------- Marquee zoom handling (on viewport) ----------
    ...
    def _apply_marquee_zoom(self, rect: QRect):
        """Zoom so that the selected rectangle roughly fills the viewport."""
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() == Qt.Key_Escape and self._full_screen:
...
- Click the **“1:1”** button’s drop‑down arrow to see the full Adobe‑style menu.
- “Show cover page” is present but disabled like in Acrobat.
- “Read mode” hides menu bar, both toolbars, and the bookmark dock.
- “Full screen mode” toggles full‑screen; Esc exits.
- “Marquee zoom” lets you drag a rectangle over the page; on mouse release the view zooms into that region.