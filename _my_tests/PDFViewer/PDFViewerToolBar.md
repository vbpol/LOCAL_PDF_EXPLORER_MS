Below is an updated, self‑contained module that adds a **dockable vertical toolbar**:

- Class **`PDFViewerToolBar`** (subclass of `QDockWidget`):
  - Current page (editable spinbox) → go to page
  - Total pages label
  - Page up / down
  - “1:1” (actual size) button
  - Zoom + / Zoom –
  - Middle‑mouse + wheel over the page → zoom in/out
  - Emits signals so it’s reusable with any viewer.

- Integrated into the previous `clsPDFBMViewer` / `PDFBMViewer` code:
  - The toolbar docks on the **left** by default, can be moved/hidden.
  - Automatically updates when a PDF is loaded / page changes.
...
- A reusable `PDFViewerToolBar` dock that behaves like Adobe’s side page/zoom bar.
- Editable current page, total pages, page up/down.
- “1:1” (actual size) zoom button.
- Zoom + / – buttons.
- Middle mouse button + wheel over the page to zoom in/out.
- Fully integrated into the existing `PDFBMViewer` / `clsPDFBMViewer` architecture.