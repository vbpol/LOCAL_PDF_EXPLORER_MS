Below is an updated, complete `PDFViewerToolBar.py` that:

- Keeps `clsPDFBMViewer` (PDF + bookmarks viewer).
- Refactors `PDFViewerToolBar` as a dockable, scalable class.
- Adds a **view/zoom menu** like Adobe’s, attached to the `1:1` button:
  - Single‑page view / Two‑page view
  - Enable scrolling (toggle)
  - Actual size
  - Zoom to page level (fit page)
  - Fit to width
  - Fit height
  - Fit visible content (same as fit width here)
- Dock can be moved to any side (left/right/top/bottom).
- Implements:
  - `set_view_mode(single/two)`
  - `set_scrolling_enabled(on/off)`
  - `fit_to_page()`, `fit_to_width()`, `fit_to_height()`, `fit_visible_content()`
  - Two‑page display (simple: two pages side by side).

Save as `PDFViewerToolBar.py` and run:

```bash
python PDFViewerToolBar.py
```
...
from PyQt6.QtGui import QAction, QPixmap, QWheelEvent, QPainter
...
class clsPDFBMViewer(QMainWindow):
    ...
    def _init_ui(self):
        ...
        self.bookmark_dock.setAllowedAreas(Qt.AllDockWidgetAreas)
...
class PDFViewerToolBar(QDockWidget):
    ...
    - Total pages label
    - Page up / down
    - '1:1' button with drop-down view/zoom menu:
        * Single-page view
        * Two-page view
        * Enable scrolling
        * Actual size
...
class PDFBMViewer(clsPDFBMViewer):
    ...
    - Left dock: PDFViewerToolBar (page + zoom + view menu).
...
- A dockable `PDFViewerToolBar` (left/right/top/bottom) with page navigation, zoom, and the requested view/zoom menu.
- A `PDFBMViewer` main window using it together with the bookmark panel.