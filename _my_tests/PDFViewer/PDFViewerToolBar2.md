The error happens because `PDFBMViewer.__init__()` was calling `_connect_signals()` **before** `self.nav_toolbar` was created.  
I’ve reordered things and cleaned up a couple of small details.

Below is a complete, ready‑to‑run version of `PDFViewerToolBar.py`.  
Replace your file contents with this and run:
...
If you run into anything else (wheel zoom behavior, styling of the vertical bar, etc.), send a screenshot and I can tweak it.