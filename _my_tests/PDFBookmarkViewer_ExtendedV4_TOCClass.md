...
- “Integration with Other AVEVA Cloud Services and On‑Premises Products” should be **one** bookmark (not two).
- “Third‑party Software Licenses” and “AVEVA Cloud Solution Security” are visually headings even though their font size may be close to body text (but they’re in a different color), so they should also become bookmarks.

Below is an updated `PDFBookmarkModel` that fixes both:

1. **Multi‑line headings across blocks**  
   - It now keeps a single `current_title/current_level` across the whole page, not per block, so if the heading wraps to the next block (as your “Integration with … Products” does), the lines are merged into **one** bookmark.

2. **Color‑based heading detection**  
   - It estimates the **body text color** (most common color near the median font size).
   - If a line is in a **different color** than body text and its font size is not clearly smaller than body, it’s also treated as a heading, even if the size heuristic alone would skip it.
   - That picks up headings like “Third‑party Software Licenses” and “AVEVA Cloud Solution Security”, which are purple while the body is black.

The public API is unchanged, so this is a **drop‑in replacement** for the `PDFBookmarkModel` in the script I gave you earlier.

---

### Replace your existing `PDFBookmarkModel` with this version
...
### What this should fix in your screenshots

- **Integration with Other AVEVA Cloud Services and On‑Premises Products**  
  If those two lines were split into separate bookmarks before, it means they were in different “blocks”.  
  With the new logic:
  - `current_title/current_level` live for the whole page.
  - Only a non‑heading line or a blank line flushes the current heading.
  - So two consecutive heading lines of the same font/level—**even across blocks**—are merged into a single bookmark.

- **Third‑party Software Licenses / AVEVA Cloud Solution Security**  
  These are probably purple headings with font size similar to the body; pure size‑based detection misses them.  
  Now:
  - We find the dominant **body text color**.
  - Any line in a **different color** and with size close to or above the body size (`>= 0.9 * body_size`) is treated as a heading, even if its size does not exceed the global heading threshold.
  - Those two purple titles should therefore be included as headings, typically at the same depth as “Security Standards and Compliance” or one level below, depending on the relative font sizes.

If you still see a specific heading missing or split after dropping this in, tell me:

- the page number,
- approximately what it looks like (color, position vs. other headings),

and I can refine the heuristic further (e.g., also considering vertical position from top, or flagging ALL‑CAPS lines as extra candidates).